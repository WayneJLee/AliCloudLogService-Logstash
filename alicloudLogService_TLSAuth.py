...
class SyncData(ConsumerProcessorBase):
    ...
    def __init__(self, target_setting=None):
        ...
        self.client_cert = self.option.get('client_cert', None)
        self.client_key = self.option.get('client_key', None)

        # try connection
        with SyslogClient(self.host, self.port, proto=self.protocol, timeout=self.timeout, cert_path=self.cert_path, client_cert=self.client_cert, client_key=self.client_key) as client:
            pass
...
def get_option():
    ...
    actionTrail_KeyId = os.environ.get('SLS_AK_ID_ActionTrail', '')
    actionTrail_Key = os.environ.get('SLS_AK_KEY_ActionTrail', '')
    ...
    for i in range(len(endpoints)):
        ...
        for logstore in logstore_list:
            ...
            if logstore == "actiontrail_all-actiontrain":
                accessKey = actionTrail_Key
                accessKeyId = actionTrail_KeyId
            if not logstore:
    ...
    # syslog options
    settings = {
                "host": "104.215.151.85", # must
                "port": 28866,       # must, port
                "protocol": "tls", # must, tcp, udp, tls (py3 only)
                "sep": "||",      # must, separator for key=value
                "cert_path": "/root/cert/cacert.pem",  # optional, cert path when TLS is configured
                "client_cert": "/root/cert/clientcert.pem",   # optional, specify path client cert
                "client_key": "/root/cert/clientkey.pem",    # optional, specify path to client key
                "timeout": 121,   # optional, default 120
